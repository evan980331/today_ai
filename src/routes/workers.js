...
router.post('/workers/:id/execute/stream', async (req, res) => {
    ...
    try {
        ...
        let ended = false;
        res.on('close', () => {
            try { require('../utils/workerDebugLog').log('workers', 'res-close', { workerId: id, ended, writableEnded: res.writableEnded, destroyed: res.destroyed, socketDestroyed: req.socket?.destroyed, signalAborted: controller.signal.aborted }); } catch {}
            ...
            if (!ended && !res.writableEnded) {
                ...
                if (req.socket?.destroyed || res.destroyed) {
                    try { require('../utils/workerDebugLog').log('workers', 'true-disconnect-abort', { workerId: id }); } catch {}
                    console.warn(`[workers] true disconnect -> abort workerId=${id}`);
                    ended = true;
                    try { controller.abort(); } catch {}
                    inflight.delete(id);
                } else {
                    try { require('../utils/workerDebugLog').log('workers', 'res-close-ignored', { workerId: id }); } catch {}
        ...
        try {
            ...
            const evDoneLogged = evDone.then(
                (v) => { try { require('../utils/workerDebugLog').log('workers', 'evDone-resolved', { workerId: id }); } catch {}; console.warn(`[workers] evDone resolved workerId=${id}`); return v; },
                (e) => { try { require('../utils/workerDebugLog').log('workers', 'evDone-rejected', { workerId: id, code: e && e.code || null, name: e && e.name || null, msg: (e && e.message || '').slice(0,200), signalAborted: controller.signal.aborted }); } catch {}; console.warn(`[workers] evDone rejected workerId=${id} code=${e && e.code} name=${e && e.name} msg=${(e && e.message || '').slice(0,120)} signalAborted=${controller.signal.aborted}`); throw e; }
            );
            const msgPLogged = msgP.then(
                (v) => { try { require('../utils/workerDebugLog').log('workers', 'msgP-resolved', { workerId: id, resultLen: (v && v.result || '').length }); } catch {}; console.warn(`[workers] msgP resolved workerId=${id} resultLen=${(v && v.result || '').length}`); return v; },
                (e) => { try { require('../utils/workerDebugLog').log('workers', 'msgP-rejected', { workerId: id, code: e && e.code || null, name: e && e.name || null, msg: (e && e.message || '').slice(0,200), signalAborted: controller.signal.aborted }); } catch {}; console.warn(`[workers] msgP rejected workerId=${id} code=${e && e.code} name=${e && e.name} msg=${(e && e.message || '').slice(0,120)} signalAborted=${controller.signal.aborted}`); throw e; }
            );
